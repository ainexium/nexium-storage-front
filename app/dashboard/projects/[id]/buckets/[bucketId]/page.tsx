...
export default function BucketPage({ params }: { params: Promise<{ id: string; bucketId: string }> }) {
  ...
  return (
    ...
    >
      {/* Drag overlay */}
      {isDragging && (
        <div className="fixed inset-0 z-50 flex flex-col items-center justify-center bg-[#0a0a0f]/90 border-2 border-dashed border-[#9b3dff]/60 pointer-events-none">
          <CloudUpload size={48} className="text-[#9b3dff] mb-3 animate-bounce" />
          <p className="text-lg font-semibold text-[#9b3dff]">{t("dropToUpload")}</p>
        </div>
      )}
      ...
      <div className="flex items-start justify-between mb-6 gap-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-1.5 bg-[#9b3dff] hover:bg-[#aa55ff] disabled:opacity-40 rounded text-sm font-medium transition-colors"
          >
            <Upload size={13} />
        ...
        <div className="mb-5">
          ...
          <div className="relative h-1 bg-gray-800 overflow-hidden rounded-full">
            {uploadPhase === "saving" ? (
              ...
              <div
                className="h-full bg-[#9b3dff] rounded-full transition-all duration-150 ease-out"
      ...
      <div className="px-2.5 py-2 flex-1 min-w-0">
        {isRenaming ? (
          <input
            ...
            className="w-full text-xs bg-[#0a0a0f] border border-[#9b3dff] rounded px-1.5 py-0.5 focus:outline-none text-white"
          />
        ) : (
          <p
          ...
          <div key={f.id} className="grid grid-cols-[auto_1fr_auto_auto_auto_auto] gap-4 items-center px-4 py-2 hover:bg-white/[0.02] group transition-colors">
            ...
            {isRenaming ? (
              <input
                ...
                onBlur={() => submitRename(f.id)}
                className="text-sm bg-[#111118] border border-[#9b3dff] rounded px-2 py-0.5 focus:outline-none w-full"
              />
            ) : (
        ...
        <div className="flex-1 overflow-auto min-h-0">
          {isLoading || !url ? (
            <div className="flex items-center justify-center h-64">
              <div className="w-8 h-8 border-2 border-gray-700 border-t-[#9b3dff] rounded-full animate-spin" />
            </div>
          ) : isImg ? (
      ...
      {range.map((p, i) =>
        p === "..." ? (
          ...
          <button
            ...
            className={`${btn} font-mono ${p === page ? "bg-[#9b3dff] text-white" : "text-gray-400 hover:text-white hover:bg-gray-800"}`}
          >
            {p}
          </button>
        )
